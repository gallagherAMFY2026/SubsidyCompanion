...
async function importFromJSON() {
  const jsonData = JSON.parse(readFileSync('/tmp/ca_programs.json', 'utf-8'));
  
  console.log(`Found ${jsonData.length} rows in JSON`);
  ...
  console.log(`\n=== IMPORT COMPLETE ===`);
  console.log(`Imported ${imported} Canadian programs`);
